# History
## 3.0.0 / 2026-06-11
First stable release of the 3.x line. Includes everything from the 3.0.0 alpha releases below — see the 3.0.0-alpha.1 entry for the summary of breaking changes since 2.x (Java 17+ baseline, PugEngine/RenderContext API, Java records support) and the README section "Breaking Changes in 3.0.0" for migration notes. The deprecated 2.x API (`PugConfiguration` and friends) keeps working in 3.0.0; removal is planned for 4.0.0.

Changes since 3.0.0-alpha-2:
* Error pages: new `PugErrorRenderer.renderHtml(exception[, generatedHtml])` renders the styled HTML error page for a `PugException` — use it in a catch block instead of `exception.toHtmlString(...)`, which is now deprecated (removal in 4.0.0). Exceptions are lean data carriers now: they capture a snapshot of the template source lines at construction time instead of holding a `TemplateLoader`; `getTemplateLines()` returns that snapshot. The `TemplateLoader`-taking exception constructors are deprecated in favor of variants taking the line snapshot. Rendering the error page uses an internal engine, so a broken user engine configuration cannot break the error page; if the error template itself fails, a minimal fallback page is returned instead of `null`.
* `RenderContext.defaultMode` now actually takes effect: for templates without a doctype, the compiler derives terse/xml rendering from the context's default mode (an explicit doctype in the template still wins). Previously the setting was silently ignored and doctype-less templates always rendered pug.js-style. The built-in default is `Mode.XHTML`, matching pug.js behavior without a doctype and the previous de-facto behavior of the new API — set `Mode.HTML` explicitly for terse output (`<input checked>`). The deprecated `PugConfiguration` path keeps its 2.x default of `Mode.HTML`.
* Expression caching moved to the expression handler: `PugEngine.Builder.expressionCacheSize(...)` (alpha-only API) is removed in favor of `new JexlExpressionHandler(cacheSize)`. The engine no longer reconfigures the handler it is given, so one handler can be passed to an engine without its cache settings being overwritten; `PugEngine.builder().caching(...)` now only controls the template cache. `PugConfiguration.setExpressionCacheSize(...)` keeps working unchanged.
* `PugException.toString()` no longer prefixes the class name with `class ` — uncaught exceptions and logs now show the conventional `de.neuland.pug4j.exceptions.PugLexerException: file:line:column` header (followed, as before, by the source context frame). Adjust log parsers matching the old prefix.
* `templateExists(...)` (PugEngine and PugConfiguration) no longer leaks the template reader.
* Deprecated `Pug4J` methods now carry `@Deprecated(since = "3.0.0", forRemoval = true)` like the rest of the deprecated 2.x API.
* Dependency updates: JEXL 3.6.2, GraalVM 25.0.3, Caffeine 3.2.4, commons-io 2.22.0, commons-lang3 3.20.0, commons-text 1.15.0, gson 2.14.0, jackson-bom 2.22.0 (test), central-publishing-maven-plugin 0.10.0
* Parser: named blocks inside conditional unbuffered code now work (pug#1589) — same-named blocks within a single template (no `extends`) are independent and each renders its own default content. Previously the parse-time extends-merge incorrectly fused them, so the last declaration won everywhere. Behavior change for templates that (accidentally) relied on the same-file merge; pug.js-conform now. Re-enables the `blocks-in-if` case for both expression handlers.
* Pretty-print: text following a filter (e.g. `include:filter`) is now indented when the filter output ends with a newline — matches pug.js, where filters run at compile time. Re-enables the `include-with-filter` case.
* Render-path performance: BlockNode child iteration no longer O(n²) (LinkedList indexed access), PugModel scope lookups halved and loop/mixin variable writes skip the redundant previous-value walk, tag classification via Set instead of linear scans, `escapeHtml4` fast path for strings without escapable characters, fewer per-render allocations. Kitchen-sink: JEXL ~13% faster, GraalJS ~16% faster (see `docs/PERFORMANCE_BASELINE.md`).
* JexlExpressionHandler: `var`/`let`/`const` keyword removal is now string-literal-aware and respects word boundaries — keywords inside strings (`'a var b'`) and identifiers (`myvar`) are no longer corrupted.
* Test suite: re-enabled previously excluded cases `code.conditionals` (passes since the buffered-code fixes), `each.else` (adjusted variant, JEXL-compatible object literals), and issue case `131` (expected HTML updated from Jade-1 to Pug-2 attribute-interpolation semantics: `#{}` in attribute string literals stays literal).
* GraalJsExpressionHandler: major performance rework — the model is now bound directly as a `with`-scope ProxyObject (the mechanism pug.js itself uses via `with (locals)`), replacing the per-expression copy of all model entries into the JS global bindings and the write-back afterwards. Variable reads/writes go straight to the PugModel. On the kitchen-sink template this makes GraalJS rendering ~48× faster (27.6 ms → 0.58 ms per render; gap to JEXL shrinks from ~185× to ~3.8×), `each` loops over records ~12× faster. See `docs/PERFORMANCE_BASELINE.md`. Limitation: program-level `'use strict'` in expressions is not supported (`with` is illegal in strict mode); function-level strict mode is unaffected.
* GraalJsExpressionHandler: record accessor-call rewrite (`person.name()` → `person.name`) is now targeted and cached — rewrites proceed leftmost-first so mixed expressions like `person.name().toUpperCase()` work, declarations and assignments in rewritten expressions survive (model write-back), and successful rewrites are cached per expression, removing the per-render exception and re-parse. The retry only triggers on not-invocable TypeErrors instead of any evaluation failure.
* RecordWrapper: removed dead `invokeMember`/`__isRecordWrapper` code, cached no-arg method reflection lookups per record class, deterministic component order (LinkedHashMap)
* Added kitchen-sink integration test (`KitchenSinkIntegrationTest`): one template exercising the full pug feature set with model-driven expressions, asserted to render identically under JEXL and GraalJS; doubles as performance workload (see `docs/PERFORMANCE_BASELINE.md` — baseline: GraalJS currently ~185× slower than JEXL on this template, rework planned)
