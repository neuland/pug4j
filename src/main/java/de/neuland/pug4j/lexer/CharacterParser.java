...
public class CharacterParser {
  ...
  private static final Pattern pattern = Pattern.compile("^\\w+\\b");
  ...
  public State parse(String src, State state, CharacterParserOptions options) {
    ...
    while (index < end) {
      try {
        ...
        CharacterParserException characterParserException =
            new CharacterParserException("Character must be a string of length 1");
  ...
  public Match parseMax(String src, CharacterParserOptions options) throws SyntaxError {
    if (options == null) options = new CharacterParserOptions();
    int start = options.getStart();
    int index = start;
    State state = this.defaultState();
    while (state.getRoundDepth() >= 0
        && state.getCurlyDepth() >= 0
        && state.getSquareDepth() >= 0) {
      if (index >= src.length()) {
  ...
  //      if (prop === undefined) {
  //        throw new Error('Bracket specified (' + JSON.stringify(bracket) + ') is not one of ")",
  // "]", or "}";');
  //      }
  //      while (state[prop] >= 0) {
  //        if (index >= src.length) {
  ...
  private int getStateProp(State state, char bracket) {
    if (')' == bracket) return state.getRoundDepth();
    if ('}' == bracket) return state.getCurlyDepth();
    if (']' == bracket) return state.getSquareDepth();
    return -1;
  }
  ...
  public Match parseMaxBracket(String src, char bracket, CharacterParserOptions options)
      throws SyntaxError {
    if (options == null) options = new CharacterParserOptions();
    int start = options.getStart();
    int index = start;
  ...
  public Match parseUntil(String src, String delimiter, CharacterParserOptions options) {
    ...
    while (index < src.length()) {
      if ((options.isIgnoreNesting() || !state.isNesting(options))
  ...
  //        state.squareDepth--;
  //      }
  //      if (!state.blockComment && !state.lineComment && !wasComment) state.history = character +
  // state.history;
  //      state.lastChar = character; // store last character for ending block comments
  //      return state;
  //    }
  public State parseChar(char character, State state) {
    //            if (character. !== 1) throw new CharacterParserException("Character must be a
    // string of length 1");
    if (state == null) state = this.defaultState();
  ...
  public boolean isKeyword(String id) {
    return ("if".equals(id))
        ...
        || ("private".equals(id))
        || ("interface".equals(id))
        || ("instanceof".equals(id))
        || ("implements".equals(id))
        || ("protected".equals(id))
        || ("public".equals(id))
        || ("static".equals(id));
  }
  ...
  //      if (/^\w+\b/.test(history) &&
  // isKeyword(/^\w+\b/.exec(history)[0].split('').reverse().join(''))) return true;
  //
  //      return false;
  //    }
