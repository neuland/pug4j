...
public class Parser {
  ...
  public Parser(String filename, TemplateLoader templateLoader, ExpressionHandler expressionHandler)
      throws IOException {
      ...
      if (peek() instanceof Newline) {
        ...
        if (expr != null) {
          if (expr instanceof BlockNode
              && !((BlockNode) expr).isYield()
              && !((BlockNode) expr).isNamedBlock()) {
  ...
  private Node parseExpr() {
    ...
    if (token instanceof Text
        || token instanceof InterpolatedCode
        || token instanceof StartPugInterpolation) {
  ...
  private Node parseMixin() {
    ...
    if (peek() instanceof Indent) {
      ...
      throw error(
          "MIXIN_WITHOUT_BODY",
          "Mixin " + mixinToken.getValue() + " declared without body",
          mixinToken);
    }
  }
  ...
  private Node parseCall() {
    ...
    if (callNode.hasBlock() && callNode.getBlock().getNodes().isEmpty()) callNode.setBlock(null);
  ...
  private Parser createParser(String templateName) {
    ...
    try {
      String resolvedPath =
          pathHelper.resolvePath(filename, templateName, templateLoader.getBase());
  ...
  private Node parseText(boolean block) {
    ...
    else return initBlock(lineno, tags);
  }
  ...
  private Node tag(AttrsNode tagNode, boolean selfClosingAllowed) {
    ...
    if (tagNode.isTextOnly()) {
      Node block = this.parseTextBlock();
      if (block == null) block = emptyBlock(tagNode.getLineNumber());
  ...
  private Node parseConditional() {
    ...
    IfConditionNode main =
        new IfConditionNode(conditionalToken.getValue(), conditionalToken.getStartLineNumber());
  ...
  private Node parseCode(boolean noBlock) {
    ...
    if (noBlock) return codeNode;
    boolean block;
    block = peek() instanceof Indent;
    if (block) {
      if (token.isBuffer()) {
        throw error(
            "BLOCK_IN_BUFFERED_CODE", "Buffered code cannot have a block attached to it", peek());
      }
  ...
  private <T extends Token> T expect(Class<T> expectedTokenClass) {
    ...
    if (t.getClass().equals(expectedTokenClass)) {
      ...
      throw error(
          "INVALID_TOKEN",
          "expected \"" + expectedTokenClass.toString() + "\", but got " + peek().getType() + "\"",
          peek());
    }
  }
