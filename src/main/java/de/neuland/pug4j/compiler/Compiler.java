...
public class Compiler implements NodeVisitor {
  ...
  public void visit(TagNode node, IndentWriter writer, PugModel model) {
    ...
    if (node.isSelfClosing() || (!template.isXml() && node.isSelfClosingTag())) {
      ...
      //            })) {
      //                this.error(name + ' is a self closing element: <'+name+'/> but contains
      // nested content.', 'SELF_CLOSING_CONTENT', tag);
      //            }
    } else {
      writer.append("<").append(tagName).append(tagAttributes).append(">");
  ...
  public void visit(CallNode node, IndentWriter writer, PugModel model) {
    ...
    String newname =
        (dynamic
            ? node.getName().substring(2, node.getName().length() - 1)
            : '"' + node.getName() + '"');
    try {
  ...
  public void visit(CaseNode node, IndentWriter writer, PugModel model) {
    try {
      ...
      for (Node when : node.getBlock().getNodes()) {
        if (skip
            || "default".equals(when.getValue())
            || node.checkCondition(model, when, getExpressionHandler())) {
    ...
    for (IfConditionNode conditionNode : node.getConditions()) {
      try {
        if (conditionNode.isDefault()
            || node.checkCondition(model, conditionNode.getValue(), getExpressionHandler())
    ...
    for (FilterNode filterValue : nestedFilterNodes) {
      ...
      if (filter != null) {
        result =
            filter.convert(
                result,
                node.convertToFilterAttributes(
                    getExpressionHandler(),
                    getTemplateLoader(),
                    model,
                    filterValue.getAttributes()),
                model);
      }
    }
    ...
    if (filter != null) {
      result =
          filter.convert(
              result,
              node.convertToFilterAttributes(
                  getExpressionHandler(), getTemplateLoader(), model, node.getAttributes()),
              model);
    }
  ...
  public void visit(MixinBlockNode node, IndentWriter writer, PugModel model) {
    ...
    if (nodes.size() == 1) {
      ...
      if (node1 != null) visit(writer, model, node1);
    }
  }
