...
public class CompilerTest {
  ...
  private void tryToRender(String file) throws IOException, URISyntaxException {
    Pug4J.render(
        TestFileHelper.getCompilerResourcePath(file + ".jade"), new HashMap<String, Object>());
  }
  ...
  private void run(String testName, boolean pretty, PugModel model) {
    ...
    de.neuland.pug4j.PugEngine engine =
        de.neuland.pug4j.PugEngine.builder()
            .templateLoader(loader)
            .expressionHandler(expressionHandler)
            .build();

    RenderContext context = RenderContext.builder().prettyPrint(pretty).build();
    ...
    try {
      ...
      assertEquals(
          testName, expected.trim().replaceAll("\r", ""), html.trim().replaceAll("\r", ""));
  ...
  private String readFile(String fileName) {
    try {
      return FileUtils.readFileToString(
          new File(TestFileHelper.getCompilerResourcePath(fileName)), "UTF-8");
    } catch (IOException | URISyntaxException e) {
      // e.printStackTrace();
    }
    return "";
