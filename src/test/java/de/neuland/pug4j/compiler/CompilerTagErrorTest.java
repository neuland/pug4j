...
public class CompilerTagErrorTest {
  ...
  private void run(String testName, boolean pretty, PugModel model)
    ...
    FileTemplateLoader loader =
        new FileTemplateLoader(TestFileHelper.getCompilerErrorsResourcePath(""), "jade");

    JexlExpressionHandler expressionHandler = new JexlExpressionHandler();
    PugEngine engine =
        PugEngine.builder().templateLoader(loader).expressionHandler(expressionHandler).build();
  ...
  private String readFile(String fileName) throws IOException, URISyntaxException {
    return FileUtils.readFileToString(
        new File(TestFileHelper.getCompilerErrorsResourcePath(fileName)), "UTF-8");
  }
}
